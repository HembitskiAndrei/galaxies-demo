...
void main(void){
  ...
  * snoise(vec3(0.0, y * 0.02, time * 200.0)) * (1.0 + strength * 4.0)
  ...
  float r = texture2D(textureSampler, vec2(rgbUvX + rgbDiff, vUV.y) + shake).r;
  float g = texture2D(textureSampler, vec2(rgbUvX, vUV.y) + shake).g;
  float b = texture2D(textureSampler, vec2(rgbUvX - rgbDiff, vUV.y) + shake).b;
  ...
  gl_FragColor = mix(
    ...
    vec4(r, g, b, 1.0) * (1.0 - bnMask - bnMask2) + (whiteNoise + blockNoise + blockNoise2 - waveNoise),
    glitchFactor
  );
}
